...
public class RequestUtils {
    ...
    private static final BiFunction<Exception, HttpStatus, String> errorReason = (exception, httpsStatus) -> {
        if (httpsStatus.isSameCodeAs(FORBIDDEN)) {
            return "You do not have enough permission";
        }
        if (httpsStatus.isSameCodeAs(UNAUTHORIZED)) {
        ...
        if (httpsStatus.is5xxServerError()) {
